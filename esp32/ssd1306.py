...
class SSD1306(framebuf.FrameBuffer):
    ...
    def init_display(self):
        for cmd in (
            ...
            SET_NORM_INV,        # non-inverted display
    ...
    def show(self):
        ...
        x1 = self.width - 1
        if self.width == 64:
            # Some 64-pixel-wide displays need a horizontal offset.
            x0 += 32
            x1 += 32
...
class SSD1306_I2C(SSD1306):
    def __init__(self, width, height, i2c, addr=0x3C, external_vcc=False):
        ...
        if self.addr not in devices:
            print("Warning: No I2C device found at address", hex(self.addr),
                  "Found devices:", [hex(d) for d in devices])
